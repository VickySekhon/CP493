        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for i, future in enumerate(as_completed(future_to_page), start=1):
                ...
                if i % 10 == 0 or i == len(page_ids):
                    print(f"Processed {i}/{len(page_ids)} pages (max = {max_pages})...")
...
def main(indicator):
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of worker threads for page-level parallelism",
    )
    ...
    print(
        f"Running with top-k={top_k}, max-pages={max_pages}, output={output}, workers={workers}"
    )
    print(f"Using **{indicator}** as an indicator of CER")
    predict_subset(top_k, max_pages, output, workers)
